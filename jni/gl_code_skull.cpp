...
GLuint loadRAWTexture(int inWidth, int inHeight, int inDepth) {
	...
	//inFileName
	if( inFile = fopen("sdcard/skull.raw", "r" )  ){
...
static const char gFragmentShader[] =
    ...
    "   texcoord.x = v_texcoord.x*0.0625;\n"
    "   texcoord.y = v_texcoord.y*0.0625;\n"
    "   int slice = int(v_texcoord.z);\n"
    "   int line = int(slice/16);\n"
    "   int row = slice-line*16;\n"
    "   texcoord.x = texcoord.x + float(row)*0.0625;\n"
    "   texcoord.y = texcoord.y + float(line)*0.0625;\n"
    "   mediump vec2 texCol;\n"
...
static const char gFragmentShader[] =
	...
	"texcoord2.x = v_texcoord2.x*0.0625;\n"
	"texcoord2.y = v_texcoord2.y*0.0625;\n"
	"slice = int(v_texcoord2.z);\n"
	"line = int(slice/16);\n"
	"row = slice-line*16;\n"
	"texcoord2.x = texcoord2.x + float(row)*0.0625;\n"
	"texcoord2.y = texcoord2.y + float(line)*0.0625;\n"
...
static const char gFragmentShader[] =
	...
	"line = int(slice/16);\n"
	"row = slice-line*16;\n"
	"texcoord3.x = v_texcoord.x*0.0625 + float(row)*0.0625;\n"
	"texcoord3.y = v_texcoord.y*0.0625 + float(line)*0.0625;\n"
	...
	"line = int(slice/16);\n"
	"row = slice-line*16;\n"
	"texcoord4.x = v_texcoord2.x*0.0625 + float(row)*0.0625;\n"
	"texcoord4.y = v_texcoord2.y*0.0625 + float(line)*0.0625;\n"
...
bool setupGraphics(int w, int h/*, const char* apkPath*/) {
    ...
    textureId=loadRAWTexture(256,256,256);
...
void renderFrame() {
    ...
    esTranslate(&modelView, 0.0f , 0.0f, -2.5f);
