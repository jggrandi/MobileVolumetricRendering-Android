...
//	textureData = (short*)malloc(sizeof(short) * texWidth * texHeight);
	...
	for( int i = 0; i < depth; i++ ){
		for( int j = 0; j < widthxheight; j+=2 ){
			//int j2 = j/2;
			...
			//textureData[coord] = imageData[i][j];
	...
	LOGE("1");
	glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, texWidth, texHeight, 0, GL_RGBA, GL_HALF_FLOAT_OES/*GL_UNSIGNED_BYTE GL_UNSIGNED_SHORT_4_4_4_4*/, textureData);
	LOGE("2");
...
bool setupGraphics(int w, int h/*, const char* apkPath*/) {
	...
	GLint iUnits;
	glGetIntegerv(GL_HALF_FLOAT_OES, &iUnits);
	LOGE("MAX TXT:%d",iUnits);
